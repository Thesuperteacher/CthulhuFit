...
const currentSessionSlice = createSlice({
  ...
  reducers: {
    ...
    updateRpeForSet: targetedSessionAction(
      (
        session,
        action: {
          exerciseIndex: number;
        ...
        // If set doesn't exist, we can't save RPE yet as it belongs to RecordedSet.
        // Option: Initialize set? But we don't know reps.
        // We'll only update if set exists.
        if (exercise.potentialSets[action.setIndex].set) {
          exercise.potentialSets[action.setIndex].set!.rpe = action.rpe;
        }
      },
    ),
